...
public class RegisterService {
	...
	public List<Object[]> findByRoutesStageByPilgrim(){
		isPilgrim(LoginService.getPrincipal());
		UserAccount userAccount=LoginService.getPrincipal();
		Pilgrim pilgrim= pilgrimService.findByUserAccount(userAccount);
		List<Object[]> result=new ArrayList<Object[]>();
		Collection<Register> registers=registerRepository.findByPilgrimId(pilgrim.getId());
		for(Register r:registers){
			Object[] object = new Object[2];
			object[0]=r.getRoute();
			object[1]=stageService.findByRouteId(r.getRoute().getId());
			result.add(object);
		}
		return result;
	}
