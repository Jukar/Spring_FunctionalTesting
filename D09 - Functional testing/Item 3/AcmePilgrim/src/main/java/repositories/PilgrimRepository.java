...
public interface PilgrimRepository extends JpaRepository<Pilgrim,Integer>{
	...
	Collection<Pilgrim> listPilgrimBirthDateDesc();
	
	@Query("select p from Pilgrim p where p.bookings.size=(select max(p.bookings.size) from Pilgrim p) group by p")
	Collection<Pilgrim> findPilgrimMoreBookings();
}
