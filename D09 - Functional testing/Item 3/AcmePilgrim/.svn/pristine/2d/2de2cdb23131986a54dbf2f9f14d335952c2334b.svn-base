...
<display:table name="complaints" id="row" requestURI="${requestURI}" pagesize="5" class="displaytag">
	...
	<display:column title="action">
		<jstl:if test="${row.status == 'OPEN'}">
			<a href="discuss/list.do?complaintId=${row.id}">
	...
	<display:column property="creationMoment"  format="{0,date,dd/MM/yyyy HH:mm}"  title="${creationMoment}" sortable="false"></display:column>
	...
	<spring:message code="complaint.reffered.name" var="name"></spring:message>
	<display:column property="reffered.name" title="${name}" sortable="false"></display:column>
	
	<spring:message code="complaint.administrator.name" var="name"></spring:message>
	<display:column property="administrator.name" title="${name}" sortable="false"></display:column>
