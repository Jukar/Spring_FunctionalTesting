...
<display:table name="anecdotes" id="row" requestURI="${requestURI}" pagesize="5" class="displaytag">
	<security:authorize access="hasRole('PILGRIM')">
		<jstl:if test="${requestURI=='anecdote/pilgrim/list.do'}">
			<display:column title="${action}" sortable="false">
				<a href="anecdote/pilgrim/edit.do?anecdoteId=${row.id}"><spring:message code="anecdote.edit"/></a>
				<br/>
			</display:column>
		</jstl:if>
	</security:authorize>
	...
	<spring:message code="anecdote.pilgrim.name" var="pilgrimname"></spring:message>
	<display:column property="pilgrim.name" title="${pilgrimname}" sortable="false"></display:column>	
	
	<spring:message code="anecdote.route.name" var="routename"></spring:message>
	<display:column property="route.name" title="${routename}" sortable="false"></display:column>
