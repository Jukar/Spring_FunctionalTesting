...
public class RouteService {
	...
	public Route create(){
		...
		Administrator admin=administratorService.findByUserAccount(user);
		Route route = new Route();
		route.setActived(true);
		route.setAdministrator(admin);
...
//		System.out.println(routeRepository.findAll());
	...
	public Collection<Route> findByAdministrator(){
		UserAccount userAccount=LoginService.getPrincipal();
		Administrator administrator=administratorService.findByUserAccount(userAccount);
		Collection<Route> routes=routeRepository.findByAdministrator(administrator);
		return routes;
	}
	...
	private void checkPrincipalSave(Route route) {
		UserAccount account=LoginService.getPrincipal();
		Administrator principal=administratorService.findByUserAccount(account); 
		Boolean res=false;
		
		if(principal.equals(route.getAdministrator())){
				res=true;
		}
		Assert.isTrue(res);
	}
